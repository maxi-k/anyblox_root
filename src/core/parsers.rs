...
/// parse iobits
pub fn tiobits<'s>(i: &'s [u8]) -> IResult<&'s [u8], u8> {
    let (i, _nbyte) = be_u32(i)?;
    let (i, _ver) = be_u16(i)?;
    // random 4 bytes for some reason, see
    // https://github.com/go-hep/hep/blob/main/groot/rtree/tree.go#L678
    let (i, _somebytes) = be_u32(i)?;
    let (i, ioflags) = be_u8(i)?;
    Ok((i, ioflags))
}
